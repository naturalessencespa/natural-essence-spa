...
(filteredProductSales || []).forEach(
  (sale: any) => {

    const product =
      sale.inventory_products?.name ||
      "Sin producto";
    ...
    productsSummary[product].quantity +=
      Number(sale.quantity || 0);

    productsSummary[product].total +=
      Number(sale.total || 0);

    productsSummary[product].commission +=
      Number(sale.commission_amount || 0);

    if (!productsSummary[product].workers[worker]) {

      productsSummary[product].workers[worker] = 0;

    }

    productsSummary[product].workers[worker] +=
      Number(sale.quantity || 0);
...
setProductSales(
  ...
  Object.entries(productsSummary).map(
    ([product, values]: any) => {

      const topWorker =
        Object.entries(values.workers)
          .sort(
            (a: any, b: any) =>
              b[1] - a[1]
          )[0]?.[0] || "-";
    ...
    📊 Exportar Excel
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
          ...
          <div className="bg-white p-6 rounded-3xl shadow">
            ...
            <h2 className="text-4xl font-bold mt-2">
              S/ {additionalSalesTotal.toFixed(2)}
            </h2>
          </div>
...
<td className="p-3 font-semibold">
  S/ {Number(item.commission).toFixed(2)}
</td>
